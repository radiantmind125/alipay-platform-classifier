...
DEFAULT_CONFIG = {
    "_说明": "SSP 上线第一版配置。阈值 2026-08-15 标定; 2026-08-21 在重搭的池子上用 lcg 取块复验通过。"
             "★ 早先这里写的是'97,610 张真图, 剔了 4,738 张' —— 那两个数**对不上任何产物**"
             "(相加得 102,348, 全仓库没有这个数), 已删。现在的口径见 _预期指标 与 _线上要盯的。",
    "line_a": {
        ...
        # ★ 默认已改为 lcg —— 2026-08-21 在整池 88,645 张上验过, 与 torch 等价:
        #     中位差 +0.000000 | 变低 47.9% / 变高 48.2% / 持平 3.9% | 判定只变 34 张(0.04%)
        #     自动拒 2.7/万(不变), 人工复核 17.4/万(对 17.8, n=154 的泊松波动 ±1.4, 不显著)
        #   选它是因为 torch 的随机数**别的语言复现不了**, 而 .NET 那条路必须逐位对得上;
        #   Python 这边也用同一套, 两边才不会各算各的。
        # ★ 已有的 ssp_config.json **不会自动跟着改** —— 删掉让它重新生成, 否则还在跑 torch。
        "patch_mode": "lcg",
        "strict": 0.8031,
        "review": 0.6497,
    ...
    "_预期指标": {
        "自动拒_每万": 2.7, "人工复核_每万": 17.4,
        "改金额覆盖_自动拒": 76.9, "改金额覆盖_复核": 95.8,
        "整图覆盖_万相": 99.4, "整图覆盖_千问": 100.0,
        "_说明": "验收时应当复现这几个数; 对不上说明装配有问题, 不是模型有问题。",
        "_人工复核这个数的来历": "三次实测: 98,013 张 torch 未定种子 = 18.4; "
                                 "88,645 张 torch 定种子 = 17.8; 88,645 张 **lcg = 17.4**(当前口径)。"
                                 "n=154 的泊松波动是 ±1.4/万, **三个数互相之间都不显著** —— "
                                 "所以 17.0~18.5 都算对得上, 别为了 0.4 去查 bug。"
                                 "自动拒那一格三次都是 2.7, 没动过。",
        "_口径依赖": "这几个数只在 line_a.patch_mode = lcg 且走 predict_seeded.py 时成立。"
                     "换回 torch 会得到另一套(差别在噪声内, 但不是同一次抽样)。",
    },
    "_线上要盯的": {
...
def load_config(path: Path) -> dict:
    ...
    # ★ patch_mode 单独查一遍。它**不是可调参数, 是算法本身** ——
    #   旧配置里这个键是**存在**的(值为 torch), 所以上面那个"缺键提醒"根本不会触发,
    #   于是会**静悄悄地继续跑没验过的那一套**。这种"键在但值旧"的情况只能挨个点名。
    pm = cfg.get("line_a", {}).get("patch_mode")
    if pm and pm != DEFAULT_CONFIG["line_a"]["patch_mode"]:
        print(f"  !!!! 配置里 patch_mode = **{pm}**, 而代码里已定为 "
              f"**{DEFAULT_CONFIG['line_a']['patch_mode']}**")
        print(f"       两者**取块算法不同, 分数不同**。确认要用 {pm} 就无视这条; "
              f"否则删掉 {path.name} 重新生成。")
    return cfg
