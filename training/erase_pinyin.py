...
def text_mask(gray: np.ndarray, bg: np.ndarray,
    ...
    ★★★★★ 更正(2026-09-24): 这里原先写过"对比度越高笔画越胖, 所以蓝底上
       拼音和汉字粘在一起, 提高阈值就能拆开"。**实测推翻了, 别照着那个思路改。**

       1. 蓝图顶部并不是全页对比度最高的字 —— 顶部约 135, 卡片里的黑字约 200,
          而黑字从来不粘。同样对比度下, 顶部和卡片的笔画外扩只差约 0.15 像素。
       2. 16 张蓝图里, 顶部拼音-汉字对粘连的比例(5.5%)和卡片里(3.9~5.6%)一样。
          顶部拼音认不出来, 主因是**大小那一关**: 顶部字比卡片字大 25~40%,
          而 big_h 是整页的 75 分位(由卡片字定), 于是顶部拼音(11~14 像素)落在
          0.55 和 0.8 倍 big_h 之间的空档, 两个桶都不收。
       3. 当初拿来举例的那一页, 顶部白字带**描黑边**(边上是纯 0,0,0 的像素,
          离底色约 106)。黑边把拼音和汉字连起来, 阈值提到 100 以下都拆不开;
          只保留"比底色亮"的像素, 顶部注音从 0 个变成 34 个。
       4. 提高阈值反而有害: big_h 跟着变小, 落进空档的顶部拼音从 20% 升到 28%;
          52 起淡色细字开始碎(白色区域 5027 块里, 52 时碎 119 块, 60 时碎 454 块)。
       全页阈值不是对的杠杆。
    """
