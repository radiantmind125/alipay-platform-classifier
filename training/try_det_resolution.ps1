...
# ★★★★★ 实测结论: **这个脚本跑不通, 包是封死的。**
#
#    2026-09-16 在服务器上实跑, 批跑脚本的 PREFLIGHT 直接拒绝:
#        Package payload size/SHA-256/reparse evidence differs:
#            models/pp/paddle_ocr_delivery.contract.json
#
#    我原以为"合同 JSON 没有内容哈希校验(模型和字典才有)" —— **那是错的**,
#    那是从代码里读出来的推断, 运行时把它否了。PREFLIGHT 对合同本身也校验
#    大小 + SHA-256 + 重解析证据, 改一个字节就拒跑。
#
#    而 det_limit_side_len **只能**从合同读(PaddleOcrDeliveryBundle.cs:612),
#    没有环境变量、没有命令行开关、没有任何覆盖口子(全分支搜过)。
#
#    ★ 所以要调这个数**只能重新签包**, 那是经理的决定。
#      这个脚本留着是因为: 万一以后拿到了能重签的路子, 改动逻辑和安全检查是现成的;
#      而且它的三道安全检查(只改副本/回头核原包/写回重读)本身验证过是对的。
#
