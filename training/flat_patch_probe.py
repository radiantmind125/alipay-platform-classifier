...
线索
----
★★ **本文件当初的猜想已被推翻, 结论也反过来了 —— 留着是因为它量到的数仍然有用。**

当初以为 SSP 取的是"最简单的那一块", 于是猜"榜首误杀是因为图太干净"。
**实际取的是纹理能量最大的那一块**(`patch_list[-1]`), 所以那个猜想不成立。

不过这个脚本量到的**相关性是真的, 而且正是它把矛盾暴露出来的**:
...
与之相符的新解释(**假说, 未独立验证**): 取的是 64 次里最花的一块 ->
纯色多的图只有少数取样命中文字, 最大值来自这少数 -> 纹理中等;
纯色少的图几乎每次都命中纹理, 最大值是 64 个里的极值 -> **更极端** -> 分数更高。
