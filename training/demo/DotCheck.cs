...
namespace Ssp
{
    ...
    /// 换成别的字体重打, 大多数字体的句点是**圆的**, 圆在方框里只占 pi/4 = 0.785。
    ///
    /// 判的是形状不是大小。同一个小数点, 量"面积 / 数字高的平方"那种做法失败过 ——
    /// 一个圆点和一个更小的方点面积可以完全一样, 拿真实圆点图去测只抓到 10%。
    ///
    /// 两侧实测:
    ///     真图 png 填充率最小 0.9121, 中位 1.0000; 服务器 78,973 张 p1 是 0.9945
    ///     圆点图 填充率最大 0.8947, 集中在 0.785 附近
    ///   **完全分开, 没有重叠。** 阈值 0.90 时真图 png 零误报。
    ///
    /// 覆盖边界, 都是实测出来的:
    ///   - **只对无损图(PNG)有效**。JPEG 会把方点的角压圆, 真图 jpg 最低到 0.7059,
    ///     和真圆点分不开。所以要调用方传 lossless=true 才判。
    ///   - **翻拍图不判**。拿相机拍屏幕会把角拍糊, 实测翻拍里 29.0% 的填充率低于 0.90,
    ///     干净截图只有 0.14%, 差 200 倍。
    ///   - **商家账单页不判**。那个页型金额字体和详情页不一样。
    ...
    {
        /// <summary>填充率低于这个值判为圆点。真图 png 最小 0.9121, 圆点最大 0.8947。</summary>
        ...
        static DotResult Check(Mat gray, DotResult res, bool lossless)
        {
            ...
            //
            // 0.90 这个阈值, 以及支撑它的 78,356 张过闸样本, 全部来自带负号的图
            // (扫描脚本本身就要求有负号)。没有负号的是收款那一类, 金额字体没量过。
            // 25,000 张对比实测: 去掉这道闸会多出 985 张(4.1%)没标过的图进来。
            //
            // 而"金额字体各类之间不一样"是已经证实的, 不是假设:
            //   商家账单页 负号 0.7368, 详情页 0.7045
            //   绿色金额   小数点填充率中位 0.8182, 其余 1.0000
            // 所以把阈值外推到没量过的那一类, 正是这个项目已经栽过的那种错。
        ...
        // 拿相机拍屏幕会把方点的角拍糊。实测翻拍里 29.0% 填充率低于 0.90, 干净截图只有 0.14%
