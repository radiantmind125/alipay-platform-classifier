...
//
// ⚠ ImageSharp 别用不带版本号的命令装 —— 会装到 4.x, 没有 license 时
// dotnet build -c Release 直接报错 No Six Labors license found(Debug 只警告)。
// 3.x 和 4.x 都是 Six Labors Split License, 商业使用义务一样, 钉 3.x 绕不开授权;
// 只有 2.x 是 Apache-2.0。3.1.5 另有 NU1903 告警。
// 要用就写死版本并先确认授权; 更省事的是用工程里已有的解码器,
// 参考 MinusCheck.cs 的接法(只收解码好的字节数组, 不依赖图像库)。
//
// ⚠ EXIF 摆正: ImageSharp 3.1.5 解码时不摆正, PIL 也不摆, 两边一致, 不用处理,
// ImageSharp 也没有开关可关。换别的解码器之前要重新确认一次。
//
// 用法
// ----
...
class OnnxCallDemo
{
    ...
    static int Main(string[] args)
    {
        ...
        foreach (var c in cases.EnumerateArray())
        {
            ...
            // 这四样都已经包在 onnx 图里了。自己再做一遍, 分数就废了, 而且不会报错。
            ...
            // ---- 4. 跑 ----
            // 正式代码里建议一次喂 4 个块而不是 16 个: 内存少占 265 MB, 速度一样,
