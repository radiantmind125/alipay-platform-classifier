...
namespace Ssp
{
    ...
    public static class TextRegion
    {
        ...
        /// </code>
        ///
        /// ★★★★★ **为什么是"撑开"不是"挪"** —— 2026-09-16 在**全部 9068 张**真图
        ///   (105,514 个带拼音的行)上量过, 三种做法:
        /// <code>
        ///     做法              通过率    覆盖中位   覆盖25分位
        ///     不动              88.5%      0.656      0.628
        ///     整个挪上去        86.1%      0.647      0.619    <- **比不动还差**
        ///     ★ 顶往上撑开      99.6%      0.958      0.923
        /// </code>
        ///   (先在 200 张上量到 89.1 / 85.8 / 99.5, 全量跑完基本一致, 没有抽样运气。)
        ///
        /// ★ 撑开之后仍有约 0.4% 的行不过 —— 那些是拼音顶得特别高的
        ///   (顶高量 95 分位是 16 像素, 而撑开是按中位 12 算的)。
        ///   要连这些也救, 得按每张图实测的顶高量来撑, 而不是用一个常数;
        ///   但那样就不是经理说的"一个数值"了, 所以先保持常数。
