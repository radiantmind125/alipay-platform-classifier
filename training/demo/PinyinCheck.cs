...
namespace Ssp
{
    ...
    /// ------------------------------------
    ///   命中 1,785 张 = 0.680%, 大约每 147 张有 1 张
    ///   准确率: 留出集 88 张(其中 52 张是贴线的)人工看, 86 张对 —— 按贴线段只占
    ///           命中的 6.8% 折算回去, 整体约 99.7%
    ///   漏判:   抽 48 张"差一点"的看, 0 张是漏的
    ///   145 张人工看过的真图, 0 误判(最高分 0.200, 线是 0.35)
    ///
    /// ★ 已知还会误判的两类:
    ///   1. **花体字**: 有些用户装了带装饰的字体, 字上方有小爱心小星星,
    ///      几何上和拼音一模一样。靠版面分不开, 要认字才行。
    ///   2. **文字很少的页**: 分母(底下有字的小块)小, 比例容易被抬高。
    ///
