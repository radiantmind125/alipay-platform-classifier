...
namespace Ssp
{
    ...
    ///
    /// 实测(本地 262,748 张, 全部人工核对过)
    /// ------------------------------------
    ///   命中 1,727 张 = 0.658%, 大约每 151 张有 1 张
    ///   准确率: 留出集 96 张(其中 60 张是贴线的)人工看, 93 张对 —— 按贴线段只占
    ///           命中的 8.4% 折算回去, 整体约 99.6%
    ///   漏判:   抽 48 张"差一点"的看, 8 张确实带拼音 -> 召回约 94%
    ///   145 张人工看过的真图, 0 误判(最高分 0.147, 线是 0.32)
    ///
    /// ★ 已知还会误判的一类: **花体字**。有些用户装了带装饰的字体,
    ///   字的上方有小爱心、小星星之类, 几何上和拼音一模一样(小块紧贴在字顶上)。
    ///   留出集里 3 张误判有 2 张是这个。靠版面分不开, 要认字才行。
    ///
    ...
    public static class PinyinCheck
    {
        ...
        /// ★ 这条和 <see cref="Threshold"/> 是**配套**的: 收紧间距之后, 没拼音的图
        ///   分数整体下移, 所以线也跟着从 0.35 降到 0.32 —— 这样既挡住了误判,
        ///   又把原来被 0.35 砍掉的那部分真图收了回来。改一个就要重新定另一个。
        /// </summary>
        public const double GapMax = 0.50;
