...
//   但页面上那条**横向分隔线**会被当成负号量。实测抽了 14,906 张蓝图:
//     · 约 **1/304** 会被量出一个 bar_width
//     · 而这些的 bar_width 是 **8.45 ~ 12.13**(正常是 0.70) —— **100% 会判成 Suspicious**
//     · 数字高 68~120, **60 px 那道闸一张都拦不住**
//   蓝图池 12 万张量级, 折合**四百张左右金额被无故清空**。
//   **这一版开头就把蓝图整个跳掉。**
//
...
//
// ★ 但**别把这说成"永远一张不差"**。放大到 9,000 张之后实测有 **13 张(0.14%)** 对不上,
//   而且**没有一张是算法本身造成的**:
//     · 9 张是**截断的 JPEG** —— PIL 直接报错不读, ImageSharp 照读, 于是 C# 这边多出了结论
//     · 4 张是 **ImageSharp 和 libjpeg 解出来的像素本来就不一样**(同一张图约 2.4% 的像素有差)
//   **同样的像素喂进去, 两边结果完全一致** —— 分歧全在解码器, 不在这个文件里的算法。
//   而且这 9,000 张里**两边报出的名单完全相同**(各 10 张)。
//   ★ 要和 Python 完全同口径, 就把截断的 JPEG 也当成读不了。
//
