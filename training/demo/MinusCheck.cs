...
namespace Ssp
{
    ...
    public static class MinusCheck
    {
        ...
        public static MinusResult Check(Mat image)
        {
            ...
            try
            {
                // ★ 16 位按 1/256 降到 8 位。真实图池里 3.32% 是 16 位 PNG(实测 30,000 张),
                //   而标定用的 cv2.imread(IMREAD_COLOR) 本身就会降位, 也就是说这些图当初
                //   就在标定集里 —— 所以这里必须降位, 不能跳过更不能抛异常。
                //
                //   和 IMREAD_COLOR **不是逐像素相同**, 别照抄这句话:
                //   ConvertTo 走的是 saturate_cast + cvRound(四舍五入), 而 IMREAD_COLOR 是截断,
                //   余数 >=128 时差 1 个灰阶。996 张 16 位图实测: 589 张有差异(最大差 1),
                //   端到端判定不同的 2 张(0.2%), 且那 2 张都是这条路量到了、IMREAD_COLOR 那条
                //   反而"定位不到金额行"。
                //   没有线性变换能表达截断(k=0 要 a<0.00196, k=255 要 a<=0.0038988, 无解),
                //   精确截断要先减低字节, 每张多三个 Mat —— 为 996 张里的 2 张不值得。
                Mat src = image;
                if (!is8)
                {
                    owned8 = new Mat();
