...
输出一段 JSON 到 **stdout**; 末尾那段汇总走 **stderr**,
所以 `> out.json` 重定向的时候汇总照样显示在屏幕上, 不会污染 JSON。
...
★ 另外可能会看到 **"原路径不存在, 已自动改用..."** 之类的告警,
说的是配置里那几个 `.pth` 训练模型路径在你们机器上没有。
**那是告警不是错误** —— 这条路走的是 ONNX, 根本不加载 `.pth`。

## 一条容易漏的判定规则

**`.jpeg` 结尾的图不自动拒, 一律转人工复核。**
