...
判据(和 dot_scan.py / MinusCheck.cs 对齐)
-----------------------------------------
  DOT_      小数点是圆的: 填充率 < 0.90, **仅 png、非翻拍、账单详情页**
            (JPEG 会把方点的角压圆; 翻拍更狠, 29% 会被压成"圆点")
  MINUSHI_  负号偏宽: >= 0.78   —— 已上线的那条
  MINUSLO_  负号偏窄: < 0.625   —— 默认没开, 但值得一起看
  BOTH_     上面两类同时命中 —— 置信度最高
  CTRL_     随机真图对照组

★ 商家账单页(左上角 X 关闭图标)一律排除 —— 那个页型金额字体不一样,
  负号天生更宽, 实测线上报出的 70% 都是它, 是误报不是问题图。

用法
----
  python training/collect_suspects.py --csv D:\probe\dot4.csv ^
      --src D:\download2\OtherImages --out D:\probe\suspect2 --controls 60
"""
...
def main() -> None:
    ...
    ap.add_argument("--controls", type=int, default=60, help="随机真图对照组拷几张")
    ap.add_argument("--max-per-kind", type=int, default=120, help="每类最多拷几张")
    ...
    for r in rows:
        try:
            mh = float(r["mh"])
            bar = float(r["bar_ratio"])
            fill = float(r.get("dot_fill", 0) or 0)
            w, h = int(r["W"]), int(r["H"])
        except (KeyError, ValueError):
            continue
        if mh < MIN_DIGIT_HEIGHT:
            continue
        if r.get("page") == "close":          # 商家账单页, 字体不同, 不算
            continue
        ok.append(r)
        ...
        if bar >= BAR_HIGH:
            ...
            why.append("MINUSLO")
        # 小数点只在 png 且非翻拍上算
        if (fill and fill < DOT_FILL_LOW
                and r.get("fmt") == "png" and not is_photo(w, h)
                and r.get("page") == "back"):
            why.append("DOT")
        if why:
            tag[r["name"]] = why
    ...
    kinds = Counter("+".join(v) for v in tag.values())
    for k, c in kinds.most_common():
        print(f"    {k:<16} {c}")

    both = [n for n, v in tag.items() if len(v) >= 2]
    print(f"★ 两类同时命中的 {len(both)} 张, 置信度最高")
