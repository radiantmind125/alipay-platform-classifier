...
所以分三组:
  ...
  A 贴线的命中   比例 0.32~0.40 全看        -> 最容易出错的一段
  B 随机命中     从全部命中里随机抽         -> 用来估准确率(无偏)
  C 差一点的     比例 0.20~0.32 随机抽      -> 用来找漏判
...
def main():
    ...
    def hit(r):
        return r['pinyin_ratio'] >= 0.32 and r['stacked'] >= 15 and r['flat'] >= 0.15
    ...
    # A: 贴线的命中, 全看
    A = sorted([r for r in hits if r['pinyin_ratio'] < 0.40],
