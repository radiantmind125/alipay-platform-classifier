...
★★ 2026-08-21 实跑结果与**必须一起说的解释**
------------------------------------------
六个集合跑下来 **元数据命中全是 0/150**, 于是"带C"和"无C"两列**完全一样**。

**这不能解读成"线路C 没用"。** 原因是**我们自己造样本的管线把元数据洗掉了**:

  `gen_api_local_edit.py:253`  `Image.open(io.BytesIO(out_bytes)).convert("RGB")`
      -> 把接口返回的字节解成像素, **元数据到此为止**
  `gen_api_local_edit.py:282`  `Image.fromarray(out).save(..., quality=95)`
      -> 重新存一张干净 JPEG, **一点元数据都没有**
  局部改金额那条更彻底: `_feather_paste` 把生成的那块**贴回原截图**,
      成品绝大部分像素是原图, 就算接口返回带标记也留不下来。

**所以这批样本天然测不出线路C。** 线路C 的真实价值另有证据:
真图池里**靠元数据翻出过 7 张真的 AI 假图**, 而且反向对照那张真实进件确实带 C2PA。

**好的一面**: 既然这批样本本来就没有元数据, 那么上面量到的检出率
**本身就是"不靠元数据"的数** —— 正是我们想要的那个诚实口径, 不需要再打折。
...
DEFAULT_SETS = [
    ...
    ("heldout_qwen26",    "千问2026(版式待确认, 见下注)",     "未训"),
    ("heldout_q26b",      "千问2026(版式待确认, 见下注)",     "未训"),
    ("nulledit",          "空编辑对照 白(未经任何模型)",      "对照"),
    ("nulledit_blue",     "空编辑对照 蓝(未经任何模型)",      "对照"),
]
# ★ 千问2026 那两个目录的版式**没有任何文档记载**, 上一版这里的"白/蓝"是我猜的。
#   实测 heldout_q26b 是 62.0%/83.3%, 与 DEPLOY_SPEC 里"千问2026 白 n=138 -> 67.4%/90.6%"
#   对得上; 而 heldout_qwen26 只有 2.0%/6.7%, **在 DEPLOY_SPEC 里根本没有对应行**。
#   确认之前不要给这两个贴版式标签。
